...
OWON produces a line of desktop multimeters that use the SCPI communication protocol via USB connection. The tool is tested with XDM1241 model. This is a 55000 count multimeter with 1µV resolution. Its fast on paper but may be painfully slow in practice especially in capacitance mode. The SCPI implementation on this device is rather primitive. For example, it is impossible to read data at the rate at which it is actually measured. Other similar models like XDM1041 should work with this tool as well.
...
Please note that Windows is not very clever at zip file unpacking. In case you are using 'Extract All...' context menu item it will attempt to create yet another folder with the same name as the archive, which you probably don't want since there is already a dmm-tools folder in the archive. So please first remove the last unnecessary folder name from the path prompt. Besides, the extraction is slow especially if you are overwriting existing installation on update. If 7-Zip tool is installed, it provides an 'Extract Here' context menu option, which is much faster and works correctly out of the box.
